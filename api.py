"""
api.py — FastAPI REST API  (Fixed v2)
======================================
Fix summary:
  - /predict/windows and /analyze/irregularity: ovulation now uses
    period_start + (cycle_length - 14), NOT period_start + cycle_length + (cycle_length - 14)
  - Fertile window clamped: never overlaps menstruation, always starts
    7+ days after period_start  (Menstruation → Follicular → Fertile → Ovulation order)
  - build_windows() replaced with build_windows_corrected() using the same
    formula as ImprovedPeriodPredictor
  - /predict/from-dates delegates entirely to ImprovedPeriodPredictor (already fixed)
...
Endpoints:
  ...
  POST /predict/from-dates    → Full prediction from raw period dates
  POST /insights/phase        → Personalised lifestyle insights by cycle phase
  POST /safety/check-toxicity → Toxicity detection (women's safety)
  GET  /health                → API health check

Run:
    pip install fastapi uvicorn joblib scikit-learn xgboost pydantic scipy
...
from period_predictor import ImprovedPeriodPredictor, PeriodEntry
...
app = FastAPI(
    ...
    description=(
        "AI-powered menstrual health predictions — PCOS detection, cycle length, "
        "fertile windows & irregularity analysis.\n\n"
        "**Phase order:** Menstruation → Follicular → Fertile Window → Ovulation → Luteal → PMS\n\n"
        "**Ovulation formula:** period_start + (cycle_length − 14 days)"
    ),
    version="2.0.0",
)
...
# ══════════════════════════════════════════════════════════════════════════════
# CONSTANTS (mirrors period_predictor.py)
# ══════════════════════════════════════════════════════════════════════════════
...
class PCOSInput(BaseModel):
    age:                    float = Field(..., ge=10,  le=60,  example=26)
    bmi:                    float = Field(..., ge=10,  le=60,  example=24.5)
    cycle_length:           float = Field(..., ge=15,  le=90,  example=35)
    cycle_regularity:       int   = Field(..., ge=2,   le=4,   example=4, description="2=regular, 4=irregular")
    follicle_no_right:      float = Field(..., ge=0,   le=40,  example=12)
    follicle_no_left:       float = Field(..., ge=0,   le=40,  example=11)
    amh:                    float = Field(..., ge=0,   le=20,  example=4.5, description="AMH level ng/mL")
    fsh:                    float = Field(..., ge=0,   le=200, example=5.2, description="FSH mIU/mL")
    lh:                     float = Field(..., ge=0,   le=200, example=8.1, description="LH mIU/mL")
    fsh_lh_ratio:           float = Field(..., ge=0,   le=10,  example=0.64)
    waist_hip_ratio:        float = Field(..., ge=0.5, le=1.5, example=0.82)
    endometrium_mm:         float = Field(..., ge=1,   le=30,  example=8.0)
    avg_follicle_size_r:    float = Field(..., ge=1,   le=40,  example=18.0)
    avg_follicle_size_l:    float = Field(..., ge=1,   le=40,  example=17.5)
    weight_gain:            int   = Field(..., ge=0,   le=1,   example=1)
    hair_growth:            int   = Field(..., ge=0,   le=1,   example=1)
    skin_darkening:         int   = Field(..., ge=0,   le=1,   example=0)
    pimples:                int   = Field(..., ge=0,   le=1,   example=1)
...
class WindowInput(BaseModel):
    ...
    period_durations:  List[float] = Field(..., min_items=3, example=[5, 5, 6, 5, 5])
    last_period_start: str         = Field(..., example="2025-12-01")
    num_cycles:        int         = Field(3, ge=1, le=6)
...
# ══════════════════════════════════════════════════════════════════════════════
# CORRECTED WINDOW BUILDER
# ══════════════════════════════════════════════════════════════════════════════

def build_windows_corrected(last_start: date, cycle_len: float,
                             avg_duration: float, num_cycles: int) -> List[dict]:
    """
    Builds cycle windows with CORRECTED phase ordering and ovulation formula.

    Phase order: Menstruation → Follicular → Fertile Window → Ovulation → Luteal → PMS

    Ovulation: period_start + (cycle_len - LUTEAL_DAYS)
    Fertile window: never overlaps menstruation, always 7+ days from period start.
    """
    windows = []
    current = last_start
    ...
    for i in range(num_cycles):
        ...
        period_end       = current + timedelta(days=pd_days - 1)
        ovulation_day    = current + timedelta(days=round(cycle_len) - LUTEAL_DAYS)
        follicular_start = period_end + timedelta(days=1)

        # Fertile window — raw, then clamped
        fertile_start_raw = ovulation_day - timedelta(days=FERTILE_DAYS_PRE)
        fertile_end       = ovulation_day + timedelta(days=FERTILE_DAYS_POST)

        earliest_fertile = max(
            follicular_start,
            current + timedelta(days=MIN_DAYS_AFTER_PERIOD_START)
        )
        ...
        if follicular_end < follicular_start:
            follicular_end = follicular_start

        luteal_start = ovulation_day + timedelta(days=1)
        next_period  = current + timedelta(days=round(cycle_len))
        luteal_end   = next_period - timedelta(days=1)
        pms_start    = next_period - timedelta(days=PMS_DAYS_BEFORE)

        windows.append({
            "cycle_number":           i + 1,
            "period_start":           str(current),
            "period_end":             str(period_end),
            "follicular_start":       str(follicular_start),
            "follicular_end":         str(follicular_end),
            "fertile_window_start":   str(fertile_start),
            "fertile_window_end":     str(fertile_end),
            "ovulation_day":          str(ovulation_day),
            "luteal_phase_start":     str(luteal_start),
            "luteal_phase_end":       str(luteal_end),
            "pms_start":              str(pms_start),
...
def health_check():
    return {
        ...
        "version":       "2.0.0",
        "phase_order":   "Menstruation → Follicular → Fertile Window → Ovulation → Luteal → PMS",
        "ovulation_formula": "period_start + (cycle_length - 14 days)",
    }
...
def predict_cycle_length(data: CycleInput):
    """Predict next cycle length from recent cycle history (min 3 cycles)."""
    ...
    return {
        ...
        "confidence":                  ("high"   if len(cl) >= 12
                                        else "medium" if len(cl) >= 6
                                        else "low"),
    }


# ── 3. Cycle Windows (CORRECTED) ──────────────────────────────────────────────
@app.post("/predict/windows", tags=["Predictions"])
def predict_windows(data: WindowInput):
    """
    Predict period, fertile window, ovulation, and luteal phase for next N cycles.

    Phase order: Menstruation → Follicular → Fertile Window → Ovulation → Luteal → PMS

    Ovulation = period_start + (cycle_length − 14 days).
    Fertile window is always AFTER menstruation ends (min 7 days from period start).
    """
    cl        = data.cycle_lengths
    pd_       = data.period_durations
    predicted = predict_next_cycle(cl)
    ...
    # Use corrected window builder
    windows = build_windows_corrected(last_start, predicted, avg_dur, data.num_cycles)
    ...
    return {
        ...
        "phase_order":             "Menstruation → Follicular → Fertile Window → Ovulation → Luteal → PMS",
        "note": (
            "Ovulation = period_start + (cycle_length − 14). "
...
# ── 4. Irregularity Analysis (CORRECTED windows) ─────────────────────────────
...
def analyze_cycle_irregularity(data: IrregularityInput):
    ...
    Windows use corrected phase ordering (Menstruation → Fertile Window → Ovulation).
    """
    ...
    # Use corrected window builder
    windows = build_windows_corrected(last_start, predicted, avg_dur, 2)
...
class PeriodDateEntry(BaseModel):
    start: str = Field(..., example="2025-09-05")
    end:   str = Field(..., example="2025-09-09")
...
class PeriodDatesInput(BaseModel):
    periods: List[PeriodDateEntry] = Field(
        ...,
        min_items=2,
        description="Period start+end dates. Min 2 months. More = better accuracy.",
        example=[
...
def predict_from_period_dates(data: PeriodDatesInput):
    """
    Simplest endpoint — provide only period start and end dates.

    Returns:
    - Next period start & end
    - Fertile window (always after menstruation)
    - Ovulation day (cycle_length - 14 from period start)
    - Follicular, luteal, PMS phases
    - Irregularity flags

    Phase order: Menstruation → Follicular → Fertile Window → Ovulation → Luteal → PMS
    Min 2 months data. Recommended: 6+ months.
    """
    try:
        entries = []
        for i, p in enumerate(data.periods):
            try:
                ...
                raise HTTPException(
                    400,
                    f"Period {i+1}: Invalid date '{p.start}' or '{p.end}'. Use YYYY-MM-DD."
                )
            try:
    ...
    try:
        predictor = ImprovedPeriodPredictor(entries)
...
# ══════════════════════════════════════════════════════════════════════════════
# PHASE HEALTH INSIGHTS
# ══════════════════════════════════════════════════════════════════════════════

class InsightInput(BaseModel):
    phase:        str                      = Field(..., example="luteal",
                                                   description="menstrual | follicular | ovulation | luteal")
    age:          Optional[int]            = Field(None, ge=10, le=60, example=28)
    symptoms:     Optional[Dict[str, float]] = Field(default={},
                                                      example={"cramps": 3, "fatigue": 4})
    health_goals: Optional[List[str]]     = Field(default=[], example=["lose weight", "improve sleep"])
    conditions:   Optional[List[str]]     = Field(default=[], example=["PCOS"])


PHASE_INSIGHTS = {
    "menstrual": {
        "icon": "🩸", "color": "#e85d75",
        "duration": "Days 1–7", "energy_level": "Low",
        "what_happens": "Uterine lining sheds. Estrogen and progesterone are at their lowest.",
        "nutrition": [
            "Iron-rich foods: spinach, lentils, red meat to replenish blood loss",
            "Vitamin C (oranges, bell peppers) to boost iron absorption",
    ...
    "follicular": {
        "icon": "🌱", "color": "#4caf82",
        "duration": "Days 6–13 (after menstruation ends)", "energy_level": "Rising → High",
        "what_happens": "Estrogen rises, follicles develop. Energy and mood naturally improve.",
        "nutrition": [
            "Lean proteins: eggs, chicken, fish to support follicle development",
            "Fermented foods (yogurt, kefir) for gut health and estrogen metabolism",
            "Flaxseeds — support healthy estrogen levels",
        ...
        "sleep": [
            ...
            "7–8 hours optimal to fuel rising energy levels",
        ],
        "avoid": [
    ...
    "ovulation": {
        "icon": "🥚", "color": "#f4a742",
        "duration": "Days 14–16 (cycle_length - 14 from Day 1)", "energy_level": "Peak",
    ...
    "luteal": {
        "icon": "🌙", "color": "#8b6fcb",
        "duration": "Days 17–28 (ovulation+1 → next period)", "energy_level": "Declining",
        "what_happens": "Progesterone rises then falls. PMS symptoms may appear in final days.",
        "nutrition": [
...
def get_personalized_insights(phase, age, symptoms, health_goals, conditions):
...
def get_phase_insights(data: InsightInput):
    """
    Personalised lifestyle suggestions for the current cycle phase.
...
# ══════════════════════════════════════════════════════════════════════════════
from toxicity_detector import ToxicityDetector, CATEGORIES
...
class ToxicityInput(BaseModel):
    ...
    context:     Optional[str] = Field(None, example="social_media",
                                       description="social_media | chat | email | comment")
    report_user: Optional[bool] = Field(False)
...
def check_toxicity(data: ToxicityInput):
    """
    AI toxicity detection for women's safety.
    Detects harassment, sexual harassment, threats, hate speech,
    body shaming, and gaslighting.
    """
...
def check_toxicity_batch(data: BatchToxicityInput):
    ...
    highest = max(results, key=lambda r: severity_order.index(r.severity))
...
def get_toxicity_categories():
    """List all toxicity categories detected."""
    return {
        "categories": [
            {
                "name":        cat,
